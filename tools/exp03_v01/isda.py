...
class ISDALoss(nn.Module):
    ...
    def isda_aug(self, linear_layer, features, labels, cv_matrix, ratio):
        ...
        C = self.class_num    # number of class
        A = features.size(1)  # feature dimension = 2048 (ResNet50)

        weight_m = list(linear_layer.parameters())[0]  # weight of Linear, shape = [200, 2048] = [C, A]

        NxW_ij = weight_m.expand(N, C, A)  # shape=[8, 200, 2048] = [N, C, A] 就是把weight_m复制了N份
        ...
        # # PYF: This is the original version, which looks confusing to me
        # sigma2 = ratio \
        #        * (weight_m - NxW_kj).pow(2).mul(
